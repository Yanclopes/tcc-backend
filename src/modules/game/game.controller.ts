...
export class GameController {
  ...
  @ApiOperation({
    summary: 'Inicia uma partida (anonima ou autenticada)',
    description:
      'Se um JWT valido for enviado, a partida fica vinculada ao usuario; caso contrario, ' +
      'a partida e anonima — o que amplia a amostra da pesquisa.',
  })
  start(
    @Body() dto: StartGameDto,
    @CurrentUser() user?: JwtUser,
  ): Promise<StartGameResponseDto> {
    return this.gameService.startGame(dto.difficultyId, user?.userId ?? null, dto.educationLevelId);
  }
